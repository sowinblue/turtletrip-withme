...
from modules.backgroud import SeaController
...
def update_todo():
    ...
    #todo !!!!! background: 데이터가 업데이트 될 때마다 달성률을 계산해서 background 컨트롤러(backgroud.py)에 보내줘야 합니다.
    current_rate = pc.calculate_completion_rate(data['tasks'])

    #todo !!!! 배경 업데이트용 데이터(rate) 보내고, 호출
    sc.calculate_and_push(current_rate)

    #todo !!!! 디버깅용 터미널프린트
    print(f"DEBUG: 현재 달성률 {current_rate}%")
